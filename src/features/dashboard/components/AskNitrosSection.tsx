...
const QUICK_QUESTIONS = ['Por que minhas vendas caíram?','Quais contatos devo priorizar hoje?','Quais conversas têm maior intenção de compra?','Quais leads estão mais quentes?','O que devo fazer para vender mais esta semana?','Quais contatos posso reativar?'];
  ...
  return <DashboardSection id="pergunte" title="Pergunte ao ChatBô" subtitle="Use o assistente existente para perguntas em linguagem natural dentro do dashboard." icon={Bot} hidden={presentationMode}>
    <div className="grid gap-4 lg:grid-cols-5">
      <div className="rounded-3xl border border-gray-200/80 bg-white/90 p-5 shadow-sm dark:border-white/10 dark:bg-gray-900/90 lg:col-span-2">
        <div className="space-y-3"><Input label="Pergunta" value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="Ex.: Quais contatos devo priorizar hoje?" /><Button type="button" className="w-full" loading={mutation.isPending} onClick={() => ask(question)}><Sparkles className="h-4 w-4"/> Perguntar ao ChatBô</Button></div>
